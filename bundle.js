...
${indentData}`);
  ...
  void main() {
    ...
    // D3 symbolDiamond's tan(30\xB0) aspect ratio (height = sqrt(3) \xD7 width).
  ...
  function createWebGLRenderer(canvas) {
    ...
    function _tick(now2) {
      const t = Math.min(1, (now2 - _animStart) / ANIM_DUR);
      const te = easeCubicInOut(t);
      const pts = _fromPts.length === _targetPts.length ? _targetPts.map((b, i) => _interpPt(_fromPts[i], b, te)) : _targetPts;
      const lines = _fromLines.length === _targetLines.length ? _targetLines.map((b, i) => _interpLine(_fromLines[i], b, te)) : _targetLines;
      _uploadPts(pts, _dpr);
    ...
    function toSVGGroup(clipId = null) {
      ...
      const root2 = document.createElementNS(ns, "g");
      root2.setAttribute("class", "webgl-export");
      ...
      root2.appendChild(cornersG);
      root2.appendChild(pointsG);
      return root2;
    }
    function clear() {
      if (_animId != null) {
    ...
    function destroy() {
      if (_animId != null) cancelAnimationFrame(_animId);
      [ptPosBuf, ptColorBuf, ptSizeBuf, ptInnerBuf, ptShapeBuf, lnPosBuf, lnColorBuf].forEach((b) => gl.deleteBuffer(b));
      gl.deleteProgram(pointProg);
      gl.deleteProgram(lineProg);
    }
    return { updatePoints, updateLines, render: render2, resize, toSVGGroup, clear, destroy, transitionTo };
  }
  ...
  var KDE_MAX_PX = 20;
  var KDE_GRID_N = 120;
  var CORNER_BOT_STRIP_Y2 = 28;
  var CORNER_LEFT_STRIP_X2 = 60;
  var SNAP_PX = 2;
  ...
  function computeKDE(vals, domainMin, domainMax) {
    ...
    return Array.from({ length: KDE_GRID_N }, (_, i) => {
      ...
      const density = vals.reduce((sum2, xi) => sum2 + Math.exp(-0.5 * ((v2 - xi) / bw) ** 2), 0) / norm;
      return { v: v2, density };
    });
  }
  function cornerMarkerPos(px, py, xScale, yScale, innerW, innerH) {
    ...
    const mx = outL ? -CORNER_LEFT_STRIP_X2 : outR ? innerW + PAD : Math.max(0, Math.min(innerW, sx));
    const my = outT ? -PAD : outB ? innerH + CORNER_BOT_STRIP_Y2 : Math.max(0, Math.min(innerH, sy));
  ...
  function computeBand(r, key, [x0, x1], nPts) {
    ...
    if (key === "ols" && r.sigma != null && r.xMean != null && r.sxx != null) {
      const t = tQ95(r.n - 2);
      return xs.map((x2) => {
        const yhat = r.intercept + r.slope * x2;
        const se = r.sigma * Math.sqrt(1 / r.n + (x2 - r.xMean) ** 2 / r.sxx);
        return { x: x2, lo: yhat - t * se, hi: yhat + t * se };
      });
    }
    if (key === "robust" && r.covIntSlope != null) {
      ...
      return xs.map((x2) => {
        const yhat = r.intercept + r.slope * x2;
        const se = Math.sqrt(r.covIntSlope[0][0] + 2 * x2 * r.covIntSlope[0][1] + x2 * x2 * r.covIntSlope[1][1]);
        return { x: x2, lo: yhat - t * se, hi: yhat + t * se };
      });
    }
    if (key === "theilsen" && r.slopeCILow != null) {
      return xs.map((x2) => ({
        x: x2,
        lo: r.intercept + r.slopeCILow * x2,
        hi: r.intercept + r.slopeCIHigh * x2
      }));
    }
    return null;
  }
  function buildColorOf(points, groupColorType, fallbackColor = "currentColor") {
    ...
    if (groupColorType === "continuous" && groupValues.length) {
      ...
      const scale2 = sequential(plasma).domain(lo === hi ? [lo - 1, hi + 1] : [lo, hi]);
      return (p) => p.group != null && isFinite(+p.group) ? scale2(+p.group) : fallbackColor;
    }
  ...
  function buildPlotModel(points, modelResult, { W, H, modelKey, groupColorType, palette }) {
  ...
  function createScatterplot(backSvgEl, frontSvgEl, overlaySvgEl, { glCanvas = null } = {}) {
    const palette = readPalette();
    const backSvg = select_default2(backSvgEl);
    const frontSvg = select_default2(frontSvgEl);
    ...
    backSvg.append("rect").attr("class", "plot-bg").attr("fill", palette.bg).attr("width", "100%").attr("height", "100%");
    const backCanvas = backSvg.append("g").attr("class", "canvas");
    const backPlotArea = backCanvas.append("g").attr("class", "plot-area").attr("clip-path", `url(#${backClipId})`);
    const xKdeEl = backPlotArea.append("path").attr("class", "kde--x").style("fill", "#aaa").style("fill-opacity", "0.35").style("stroke", "none").style("pointer-events", "none");
    const yKdeEl = backPlotArea.append("path").attr("class", "kde--y").style("fill", "#aaa").style("fill-opacity", "0.35").style("stroke", "none").style("pointer-events", "none");
    const ciBandEl = backPlotArea.append("path").attr("class", "ci-band").style("fill", palette.regline).style("fill-opacity", "0.12").style("stroke", "none").style("pointer-events", "none");
    const regLineEl = backPlotArea.append("line").attr("class", "regression-line").style("stroke", palette.regline).style("stroke-width", "1.75").style("fill", "none");
    ...
    const frontPlotArea = frontCanvas.append("g").attr("class", "plot-area").attr("clip-path", `url(#${frontClipId})`);
    const pointsG = frontPlotArea.append("g").attr("class", "points");
    const cornersG = frontCanvas.append("g").attr("class", "corners");
    ...
    let _lastPhysW = 0, _lastPhysH = 0;
    let _animatingUntil = 0;
    function update({
      points,
      modelResult,
      xLabel = "x",
      ...
      if (animate) _animatingUntil = performance.now() + 220;
      clearHover();
      ...
      function drawKdeStrip(el, kdeData, scale2, orient) {
      ...
      const T = transition().duration(animate ? 200 : 0).ease(expOut);
      if (hasLine) {
      ...
      if (glRenderer) {
          ...
          if (p.corner) {
            ...
            if (p.censored) {
              ...
              glPoints.push({
                ...
                y: fullY,
                r: _pointR,
                color: cssToGL(palette.censored, alpha),
                innerRadius: innerR,
                shape: 0
              });
            } else {
        ...
        const glLines = [];
        for (const v2 of xVals) {
          if (v2 < xScale.domain()[0] || v2 > xScale.domain()[1]) continue;
          const sx = MARGIN.left + xScale(v2);
          glLines.push({
            x1: sx,
            y1: MARGIN.top + iH,
            x2: sx,
            y2: MARGIN.top + iH + TICK_LEN,
            color: fringeColor
          });
        }
        for (const v2 of yVals) {
          if (v2 < yScale.domain()[0] || v2 > yScale.domain()[1]) continue;
          const sy = MARGIN.top + yScale(v2);
          glLines.push({
            x1: MARGIN.left,
            y1: sy,
            x2: MARGIN.left - TICK_LEN,
            y2: sy,
            color: fringeColor
          });
        }
      ...
      cornersOverlayG.selectAll(".point--corner").data(cornerPoints, (d) => d.index).join(
        ...
        (update2) => update2,
        (exit) => exit.remove()
      ).attr("transform", (d) => `translate(${d.corner.x},${d.corner.y})`).attr("d", Symbol2().type(diamond_default).size(60)).attr("data-cx", (d) => d.corner.outB ? d.corner.x : null).attr("data-cy", (d) => d.corner.outL ? d.corner.y : null).style("fill", "none").style("stroke", "none");
    ...
    function drawAxisSpine(g, len, orient) {
      g.selectAll("*").remove();
      const isX = orient === "x";
      g.append("line").classed("axis-line", true).style("stroke", "#555").style("stroke-width", "1").attr("x1", 0).attr("y1", 0).attr("x2", isX ? len : 0).attr("y2", isX ? 0 : len);
    }
    ...
    function getExportSVG() {
      ...
      root2.setAttribute("viewBox", `0 0 ${W} ${H}`);
      root2.setAttribute("width", W);
      root2.setAttribute("height", H);
      root2.setAttribute("xmlns", ns);
      for (const child of backSvgEl.children) root2.appendChild(child.cloneNode(true));
      if (glRenderer) root2.appendChild(glRenderer.toSVGGroup(backClipId));
      for (const child of frontSvgEl.children) root2.appendChild(child.cloneNode(true));
      return root2;
    }
  ...
  function createDiagnostics(container, overlayContainer, { glCanvas = null, onQQHover = null } = {}) {
    ...
    function update({ residuals: residuals2, activeIndices, hoveredIndex: hoveredIndex2 = null, pointColors = null }) {
      ...
      if (residuals2 !== lastResiduals || activeIndices !== lastActiveIndices) {
        ...
        diagState = fullDraw(svg, overlaySvg, residuals2, activeIndices, pointColors, palette, useGL);
        if (glRenderer && diagState) {
          const { width: W, height: H } = container.getBoundingClientRect();
          glRenderer.resize(Math.round(W * dpr), Math.round(H * dpr));
          const glLines = residuals2.map((r) => ({
            x1: diagState.sharedAxisX - 4,
            y1: diagState.m.top + diagState.yScale(r),
            x2: diagState.sharedAxisX,
            y2: diagState.m.top + diagState.yScale(r),
            color: cssToGL2(palette.muted, 0.4)
          }));
          const glPoints = diagState.sortedWithIdx.map(({ r, i }, si) => ({
            ...
            r: 2.5,
            color: cssToGL2(pointColors ? pointColors[i] : palette.point, 0.7),
            innerRadius: 0
          }));
    ...
    function applyGroupDim() {
      ...
      qqGroupDimG.append("rect").attr("x", 0).attr("y", 0).attr("width", qqPanelW).attr("height", iH).attr("fill", palette.bg).attr("opacity", 0.72).style("pointer-events", "none");
      qqHitPoints.forEach((p, si) => {
        if (!_groupHoverSet.has(p.rowIndex)) return;
        const color2 = pc ? pc[sortedWithIdx[si].i] : palette.point;
        qqGroupDimG.append("circle").attr("cx", p.localX).attr("cy", p.localY).attr("r", 2.5).attr("fill", color2).attr("opacity", 0.7).style("pointer-events", "none");
      });
    }
    function clear() {
      svg.selectAll("*").remove();
  ...
  function fullDraw(svg, overlaySvg, residuals2, activeIndices, pointColors, palette, useGL = false) {
    ...
    const { sortedWithIdx, theoretical, xScale: qqXScale } = drawQQ(qqG, residuals2, n, iH, yScale, panelW, pointColors, palette, useGL);
  ...
  function drawQQ(g, residuals2, n, iH, yScale, panelW, pointColors, palette, useGL = false) {
  ...
  var _cachedUpdateArgs = null;
  var hoveredIndex = null;
  var currentPointColors = null;
  var currentPoints = null;
  var currentActiveIndices = null;
